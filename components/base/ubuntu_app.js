...
export class UbuntuApp extends Component {
    ...
    render() {
        return (
            <div
                className="p-1 m-px z-10 bg-white bg-opacity-0 hover:bg-opacity-20 focus:bg-ub-orange focus:bg-opacity-50 focus:border-ubb-orange focus:border-opacity-100 border border-transparent outline-none rounded select-none w-24 h-20 flex flex-col justify-start items-center text-center text-xs font-normal text-white relative"
                id={"app-" + this.props.id}
                onDoubleClick={this.openApp}
                onClick={() => {
                    // touch screens have no real double click, so a single tap opens the app
                    if (window.matchMedia && window.matchMedia('(pointer: coarse)').matches) this.openApp();
                }}
                tabIndex={0}
            >
