...
export class Terminal extends Component {
    constructor() {
        ...
        this.child_directories = {
            root: ["projects", "experience", "skills", "languages", "achievements", "LNMIIT", "interests", "personal-documents"],
            LNMIIT: ["Sem-7", "ACM-Student-Chapter", "TA-certificates"],
            projects: ["JobFit", "GAN-cmfd", "clinic-os", "craafter", "SyncPen", "mainak569.github.io"],
            experience: ["ICSC-Data-Analyst-Intern", "LUSIP-Research-Intern", "Teaching-Assistant", "ACM-Secretary"],
            skills: ["Full-Stack-Development", "Next.js", "React", "Node.js", "Django", "PostgreSQL", "PyTorch", "DSA"],
            languages: ["C++", "C", "JavaScript", "TypeScript", "Python", "Java"],
            achievements: ["LeetCode-Knight-1900+", "CodeChef-3-Star"],
            interests: ["Software Engineering", "Competitive Programming", "Deep Learning", "Computer Vision"],
        };
        this.available_commands = "[ help, whoami, cd, ls, pwd, echo, clear, exit, mkdir, code, spotify, chrome, about-mainak, resume, trash, settings, sendmsg ]";
        this.state = {
            terminal: [],
        }
    }
        ...
        return (
            <React.Fragment key={id}>
                <div className="flex w-full h-5">
                    <div className="flex">
                        <div className=" text-ubt-green">mainak@ubuntu</div>
        ...
        switch (main) {
            case "cd":
                ...
                if (this.child_directories[this.curr_dir_name].includes(rest)) {
                    ...
                    result = `bash: cd: ${this.xss(rest)}: No such file or directory`;
                }
                break;
            case "ls":
                let target = words[0];
                ...
                if (target in this.child_directories) {
                    ...
                    result = `ls: cannot access '${this.xss(target)}': No such file or directory                    `;
                }
                break;
            case "mkdir":
            ...
            case "pwd":
                ...
                result = str.replace("~", "/home/mainak")
                break;
            case "code":
                if (words[0] === "." || words.length === 0) {
                    this.props.openApp("vscode");
                } else {
                    result = this.notFound(main);
                }
                break;
            case "echo":
            ...
            case "about-mainak":
                if (words[0] === "." || words.length === 0) {
                    this.props.openApp("about-mainak");
                } else {
                    result = this.notFound(main);
                }
                break;
            ...
            case "sendmsg":
                ...
                break;
            case "help":
                result = `Welcome to Mainak's portfolio! 🐧<br>Try <span class="text-ubt-blue">ls</span>, <span class="text-ubt-blue">cd projects</span>, <span class="text-ubt-blue">whoami</span>, <span class="text-ubt-blue">about-mainak</span>, <span class="text-ubt-blue">resume</span> or <span class="text-ubt-blue">sendmsg</span>.<br>Available Commands: ${this.available_commands}`;
                break;
            case "whoami":
                result = "Mainak Das — Full-Stack Developer · B.Tech CCE @ LNMIIT Jaipur · LeetCode Knight · mainak.lnmiit@gmail.com";
                break;
            case "resume":
                window.open("./files/Mainak-Das-Resume.pdf", "_blank");
                result = "Opening resume in a new tab...";
                break;
            case "clear":
                this.reStartTerminal();
                return;
    ...
    notFound(command) {
        return "Command '" + this.xss(command) + "' not found, or not yet implemented.<br>Available Commands: " + this.available_commands;
    }
