...
const EMAILJS_CONFIGURED = Boolean(process.env.NEXT_PUBLIC_USER_ID && process.env.NEXT_PUBLIC_SERVICE_ID && process.env.NEXT_PUBLIC_TEMPLATE_ID);
...
export class Gedit extends Component {
    ...
    componentDidMount() {
        if (EMAILJS_CONFIGURED) emailjs.init(process.env.NEXT_PUBLIC_USER_ID);
    }
    ...
    sendMessage = async () => {
        ...
        // without EmailJS keys, hand the message to the visitor's mail client
        if (!EMAILJS_CONFIGURED) {
            const body = `${message}\n\n— ${name}`;
            window.location.href = `mailto:${CONTACT_EMAIL}?subject=${encodeURIComponent(subject || "Hello from mainak.me")}&body=${encodeURIComponent(body)}`;
            return;
        }
        ...
        return (
            <div className="w-full h-full relative flex flex-col bg-ub-cool-grey text-white select-none">
                <div className="flex items-center justify-between w-full bg-ub-gedit-light bg-opacity-60 border-b border-t border-blue-400 text-sm">
                    ...
                    {this.state.error ? <span className="text-xs text-red-300">Couldn't send — email me at <a className="underline" href={`mailto:${CONTACT_EMAIL}`}>{CONTACT_EMAIL}</a></span> : null}
                    <div className="flex">
